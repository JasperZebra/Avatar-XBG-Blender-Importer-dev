...
def remap_skin_indices(mesh, sub_mesh_list):
    """Remap bone indices from palette indices to global bone IDs
    
    Each vertex stores indices into a bone palette (0-47), not global bone IDs.
    The DNKS chunk provides the palette mapping for each submesh.
    This function converts palette indices to actual bone IDs.
    """
    if not mesh.skin_indice_list or not mesh.mat_list_info:
        return
        ...
        if lod_grp < len(sub_mesh_list):
            ...
            if submesh:
                ...
                for v_idx in range(vert_id_start, end):
                    mesh.skin_indice_list[v_idx] = tuple(
                        (palette[r] if r < len(palette) and palette[r] != -1 else 0) 
                        for r in mesh.skin_indice_list[v_idx]
                    )
                vert_id_start += count
